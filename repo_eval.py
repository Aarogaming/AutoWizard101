...
keywords_list = ["rl","reinforcement learning","imitation learning","unity","gym","training","replay","dataset","vision","ocr","yolov5","detectron","object detection","template matching","image recognition","screenshot","inventory","label","annotation","pathfinding","navigation","recast","behavior tree","py_trees","fsm","actions","gui automation","input injection","mouse","keyboard","installer","packaging","electron","pyinstaller","dashboard","streamlit","dearpygui","logging","wandb","mlflow","examples","tutorial","examples/unity","examples/unreal","windows","cross-platform","python","c#","c++","docker"]
...
def relevance_scores(meta, kws):
    ...
    if pushed:
        try:
            recent = (now - datetime.datetime.fromisoformat(pushed.replace('Z',''))).days <= 730
        except Exception:
            pass
    kw_lower = [k.lower() for k in kws]
    rl_hit = any(term in kw_lower for term in ["rl","reinforcement learning","imitation","gym","unity","training"])
    vision_hit = any(term in kw_lower for term in ["vision","ocr","yolov5","detectron","object detection","template matching","image recognition","screenshot","label","annotation","inventory"])
    gui_hit = any(term in kw_lower for term in ["gui automation","input injection","mouse","keyboard","automation"])
    path_hit = any(term in kw_lower for term in ["pathfinding","navigation","recast"])
    inv_hit = any(term in kw_lower for term in ["inventory","label","annotation","object detection"])
    installer_hit = any(term in kw_lower for term in ["installer","packaging","electron","pyinstaller"])
    dev_hit = any(term in kw_lower for term in ["logging","wandb","mlflow","docker","ci"])
    dash_hit = any(term in kw_lower for term in ["dashboard","streamlit","dearpygui","pysimplegui","ui"])
    ...
    basis_parts = []
    if rl_hit: basis_parts.append("rl keywords")
    if vision_hit: basis_parts.append("vision/ocr keywords")
    if gui_hit: basis_parts.append("gui automation keywords")
    if path_hit: basis_parts.append("pathfinding keywords")
    if installer_hit: basis_parts.append("packaging keywords")
    if dev_hit: basis_parts.append("dev/logging keywords")
    if recent: basis_parts.append("recent activity")
    if stars>1000: basis_parts.append(">1k stars")
...
def recommendation(scores, lic_cat):
    ...
    if top >= 70 and lic_cat == "permissive":
        if scores["Vision_OCR"] >= 70 or scores["GUI_Automation"] >= 70 or scores["Installer_Packaging"] >= 70 or scores["Dashboard_UI"] >= 70:
            return "use_directly"
...
def suggested_steps(name):
    steps = []
    nl = (name or "").lower()
    if any(x in nl for x in ["yolo","ultralytics","detectron","opencv","easyocr","tesseract"]):
    ...
    if any(x in nl for x in ["ml-agents","gymnasium","stable-baselines","acme","rlpyt","imitation","rl-baselines3-zoo"]):
...
for url in repos:
    ...
    try:
        ...
        text_blob = " ".join(filter(None, [entry.get("description") or "", entry.get("readme_excerpt") or ""]))
        ...
        entry["license_assessment"] = {"category": lic_cat, "reuse_allowed": True if reuse=="true" else False if reuse=="false" else "unknown"}
...
summary_counts = {"use_directly":0,"fork_and_extend":0,"study_and_extract_patterns":0,"dataset_or_tooling_only":0,"low_priority":0}
for r in results:
    rec = r.get("recommendation","low_priority")
    if rec not in summary_counts:
        summary_counts[rec]=0
    summary_counts[rec]+=1

sorted_repos = sorted(results, key=lambda x: (x.get("scores",{}).get("Ease_of_Integration",0), max(x.get("scores",{}).values() or [0])), reverse=True)
