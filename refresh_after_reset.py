...
REPORT_PATH = os.path.join(ROOT, 'ProjectMaelstrom', 'repo_analysis_report.json')
SOURCES_PATH = os.path.join(ROOT, 'ProjectMaelstrom', 'wizard101_data_sources.json')
PLAN_PATH = os.path.join(ROOT, 'project_integration_plan.json')
...
kw_map = {
    ...
    'Vision_OCR': ['vision', 'ocr', 'yolo', 'detectron', 'object detection', 'template', 'screenshot', 'image recognition', 'opencv'],
    ...
    'Inventory_Image_Categorization': ['inventory', 'dataset', 'annotation', 'label', 'cvat', 'label-studio'],
    'Installer_Packaging': ['installer', 'packaging', 'electron', 'pyinstaller', 'builder'],
    'Dev_Tools_Integrations': ['logging', 'tracking', 'wandb', 'mlflow', 'ci', 'metrics'],
    'Dashboard_UI': ['dashboard', 'ui', 'viewer', 'streamlit', 'dearpygui', 'pysimplegui']
}
...
def license_category(spdx):
    ...
    su = spdx.upper()
    permissive = {'MIT','BSD-3-CLAUSE','BSD-2-CLAUSE','APACHE-2.0','ISC','UNLICENSE'}
...
def recommendation_from_scores(scores, license_cat):
    if scores.get('Vision_OCR',0) >= 60 or scores.get('RL_training',0) >= 60:
        return 'use_directly' if license_cat in {'permissive','other'} else 'fork_and_extend'
    if scores.get('Vision_OCR',0) >= 40 or scores.get('RL_training',0) >= 40:
        return 'fork_and_extend'
    if scores.get('Installer_Packaging',0) >= 40 or scores.get('GUI_Automation',0) >= 40:
        return 'use_directly'
...
for url in repo_urls:
    ...
    if 'github.com' not in url:
        entry.update({
            ...
            'license_assessment': {'category': 'unknown', 'reuse_allowed': 'unknown'},
    ...
    try:
        ...
        entry['owner']=owner; entry['name']=name
        ...
        text_blob = desc
        scores = {axis: axis_score(axis, text_blob, meta.get('stargazers_count'), meta.get('pushed_at'), meta.get('language')) for axis in kw_map.keys()}
        scores['Ease_of_Integration'] = ease_score(text_blob, meta.get('stargazers_count'), meta.get('pushed_at'), meta.get('language'))
...
sorted_repos = sorted([r for r in repos_out if isinstance(r.get('stars'), int)], key=lambda x: x.get('stars',0), reverse=True)
...
plan = {
    ...
    'milestones_90d': [
        {'phase': '0-30d', 'goals': ['Lock legal/ethics guardrails in code (dev mode logging).', 'Grow template/OCR library for HUD stats; measure accuracy baseline.', 'Add script catalog with credits + provenance display.', 'Prototype diagnostics dashboard.']},
        {'phase': '31-60d', 'goals': ['Integrate behavior trees/FSM for SmartPlay task orchestration.', 'Graph/path routing for common hubs using public maps.', 'Add portable + installer build matrix and smoke tests.', 'Seed dataset workflow with consented captures.']},
        {'phase': '61-90d', 'goals': ['Optional YOLOv5 detector prototype.', 'Optional offline RL experiments (no live game).', 'UI polish + accessibility + About/Credits expansion.', 'Publish contributor guide and auto-credit pipeline.']}
    ],
}
